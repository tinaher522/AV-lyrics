...
def gen_wavscp_utt2spk_files(text):
    ...
    for line in text:
        if line[12:16] == '0101':
            path_audio = "video/mp4_segments/"
        else:
            path_audio ="video/mp4_modifications/"
        
        wav_scp.append(line.split()[0] + " " + path_audio + \
            line[0] + "/" + line[:4] + "/" + line.split()[0] + ".mp4")
...
def main():
    ...
    with open(path_to_recipe + '/data/train_video/spk2gender', 'wb') as f:
        f.write("\n".join(spk2gender_train))
    with open(path_to_recipe + '/data/test_video/spk2gender', 'wb') as f:
        f.write("\n".join(spk2gender_test))
    
    with open(path_to_recipe + '/data/train_video/wav.scp', 'wb') as f:
        f.write("\n".join(wav_scp_train))
    with open(path_to_recipe + '/data/test_video/wav.scp', 'wb') as f:
        f.write("\n".join(wav_scp_test))     

    with open(path_to_recipe + '/data/train_video/text', 'wb') as f:
        f.write("\n".join(text_train))
    with open(path_to_recipe + '/data/test_video/text', 'wb') as f:
        f.write("\n".join(text_test))    
    
    with open(path_to_recipe + '/data/train_video/utt2spk', 'wb') as f:
        f.write("\n".join(utt2spk_train))
    with open(path_to_recipe + '/data/test_video/utt2spk', 'wb') as f:
        f.write("\n".join(utt2spk_test)) 
