...
Usage:
  refine.py  <segfilenm> [<inwavroot>] [<outsegfilenm>]
  refine.py --help

Options:
  <segfilenm>      Name of the segmentation file
  <inwavroot>      Name of the root dir for the input audio file. Defaults to '../../data/16khz16bit'.
...
class Model:
    def __init__(self, seg_filenm, in_root, out_seg_filenm):
    ...
    def play(self, record_id, start_offset=0, length=None, tempo=None):
        ...
        wav_filename = os.path.join(self.in_root, '{}.wav'.format(self.segments[record_id]['wavfile']))
...
class RecordList(npyscreen.MultiLineAction):
    ...
    def display_value(self, vl):
        ...
        return "[{:3s}] [{:3d}]({:2d}) {:8.2f} - {:8.2f} :: {}".format(label,
                                                                       record['index'],
                                                                       record['repeat'],
                                                                       record['start'],
                                                                       record['end'],
...
class TranscriberApplication(npyscreen.NPSAppManaged):
    def __init__(self, segfilenm, in_root, out_seg_filenm):
        super(TranscriberApplication, self).__init__()
        self.db = Model(segfilenm, in_root, out_seg_filenm)
...
def main():
    ...
    in_root = arguments['<inwavroot>'] or '../../data/16khz16bit'
    out_seg_filenm = arguments['<outsegfilenm>'] or segfilenm

    logging.basicConfig(level=logging.DEBUG, filename='test.log')

    app = TranscriberApplication(segfilenm, in_root, out_seg_filenm)
    app.run()
