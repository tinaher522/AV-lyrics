...
class PyLyrics:
    ...
    def getAlbums(singer):
        ...
        if not_found == 0:
            als.append(Album('Other Songs (2016)','http://lyrics.wikia.com:' + singer,singer))
            als.append(Album('Compilations (2016)','http://lyrics.wikia.com:' + singer,singer))
            als.append(Album('Singles (2016)','http://lyrics.wikia.com:' + singer,singer))
    ...
    def getTracks(album):
        ...
        for al in soup.find_all('album'):
            if al.text.lower().strip() == album.name.replace(':','').strip().lower():
                currentAlbum = al
                break
        songs = []
...
def main():
    albums = PyLyrics.getAlbums('Grades')
    print (albums)
    print albums[-1]
    tracks = PyLyrics.getTracks(albums[-3])
    print (tracks[0].getLyrics())
